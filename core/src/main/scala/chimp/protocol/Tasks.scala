...
// the Tasks wire format encodes durations as an integer number of milliseconds (ttlMs, pollIntervalMs), not as ISO-8601 strings; this
// file-private codec bridges java.time.Duration to that representation without leaking into the wider protocol scope
private given Codec[Duration] =
  Codec.from(Decoder.decodeLong.map(Duration.ofMillis), Encoder.encodeLong.contramap(_.toMillis))
