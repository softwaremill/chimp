...
/** The same as [[respondToJsonRpc]], but the response is an event stream produced by the given [[ServerStreamingHttpTransport]]'s streaming
  * machinery, rather than a single JSON body.
  */
