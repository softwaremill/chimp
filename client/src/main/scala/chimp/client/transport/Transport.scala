...
/** A bidirectional transport carrying JSON-RPC messages for an MCP client.
  ...
  *  - `onIncoming(handler)` registers a callback for server-initiated requests and notifications.
  *    HTTP non-streaming transports never invoke it; stdio and streaming transports do.
  */
trait Transport[F[_]]:
